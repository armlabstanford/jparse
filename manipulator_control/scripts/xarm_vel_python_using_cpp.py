...
from manipulator_control.msg import JparseTerms, Matrow
from manipulator_control.srv import JParseSrvResponse, JParseSrvRequest, JParseSrv
...
class ArmController:
    def __init__(self):
        ...
        self.use_service_bool = rospy.get_param('~use_service_bool', False) #boolean to control if the service is used or not
        ...
        if self.use_service_bool == False:
            #in this case use the published message
            rospy.Subscriber("jparse_output", JparseTerms, self.jparse_terms_callback, queue_size=1)
        #initialize the jparse and jsafety matrices
        self.jparse_mat = None
        self.jsafety_mat = None
    ...
    def service_caller(self, service_name, request):
        """
        Calls the JParseSrv, then builds and returns two numpy matrices:
        - jparse_matrix:         shape (R, C)
        - jsafety_nullspace_matrix: shape (R, C)
        where R is the number of rows (i.e. len(response.jparse))
        and C is the number of columns (i.e. len(response.jparse[0].row)).
        """
        # wait for provider
        rospy.wait_for_service(service_name)
        try:
            call = rospy.ServiceProxy(service_name, JParseSrv)
            resp = call(request)

            # turn list<Matrow> into a 2D array
            # each Matrow.row is a list of floats
            if resp.jparse:
                jparse_mat = np.stack([ np.array(mrow.row) for mrow in resp.jparse ])
            else:
                jparse_mat = np.empty((0, 0))

            if resp.jsafety_nullspace:
                jsafety_mat = np.stack([ np.array(mrow.row) for mrow in resp.jsafety_nullspace ])
            else:
                jsafety_mat = np.empty((0, 0))

            return jparse_mat, jsafety_mat

        except rospy.ServiceException as e:
            rospy.logerr("JParseSrv call failed: %s", e)
            return None, None
        
    def jparse_terms_callback(self, msg):
        """
        msg.jparse and msg.jsafety_nullspace are lists of Matrow,
        each Matrow.row is a list of floats.
        We stack them into R×C numpy arrays.
        """
        # build jparse matrix
        if msg.jparse:
            rows = [ np.array(mrow.row, dtype=float) for mrow in msg.jparse ]
            self.jparse_mat = np.stack(rows, axis=0)
        else:
            self.jparse_mat = np.empty((0,0))

        # build safety‐nullspace matrix
        if msg.jsafety_nullspace:
            rows = [ np.array(mrow.row, dtype=float) for mrow in msg.jsafety_nullspace ]
            self.jsafety_mat = np.stack(rows, axis=0)
        else:
            self.jsafety_mat = np.empty((0,0))
    ...
    def control_loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.joint_states and self.target_pose:
                ...
                if self.use_service_bool == True:
                    #call the service to get the JParse and jsafety matrices
                    request = JParseSrvRequest()
                    request.gamma = self.jparse_gamma
                    request.base_link_name = self.base_link
                    request.end_link_name = self.end_link
                    request.singular_direction_gain_position = self.phi_gain_position
                    request.singular_direction_gain_angular = self.phi_gain_angular
                    jparse_mat, jsafety_mat = self.service_caller("/jparse_srv", request)
                    if jparse_mat is not None:
                        J_method, J_nullspace = jparse_mat, jsafety_mat
                else:
                    #use the jparse_mat and jsafety_mat from the callback
                    J_method, J_nullspace = self.jparse_mat, self.jsafety_mat

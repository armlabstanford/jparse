...
class ArmController:
    def __init__(self):
        ...
        self.base_link = rospy.get_param('/base_link_name', 'panda_1_link0') #defaults are for panda
        self.end_link = rospy.get_param('/end_link_name', 'panda_1_hand')
        ...
        self.pub_joint_states = rospy.Publisher('/control_script_joint_states', JointState, queue_size=10)
        ...
        # Joint names
        self.joint_names = [f'panda_1_joint{i}' for i in range(1, 8)]

        # Control parameters
        self.frequency = 50.0  # Hz
        # Loop duration
        self.duration = rospy.Duration(1.0 / self.frequency)
        
        self.rate = rospy.Rate(self.frequency)  # 10 Hz
        #home the robot
        self.torque_home_robot()
    ...
    def pose_error(self, current_pose, target_pose, error_norm_max = 0.05):
        """
    ...
    def nominal_joint_pose_nullspace_panda(self, q, dq, Vmax = 5, Kp_value_set=5.0, Kd_value_set=3.0, homing=False):
        """
        ...
        """
        # Nominal joint pose for the Panda robot
        if homing:
        ...
        if q_error_norm > 0.1:
            ...
            if homing:
                Kp_value = Kp_value_set #5.0
            else:
                Kp_value = Kp_value_set #5
            Kp = np.diag([Kp_value] * len(q))
            if homing:
                Kd_value = Kd_value_set #3.0
            else:   
                Kd_value = Kd_value_set #3
            Kd = np.diag([Kd_value] * len(q))

            # Compute the nominal joint motion
            nominal_joint_motion = (g/q_error_norm) * Kp@q_error - Kd@dq
            nominal_joint_motion_norm = np.linalg.norm(nominal_joint_motion)
    ...
    def publish_pose_errors(self, position_error, angular_error):
        """
    ...
    def control_loop(self):
        ...
        while not rospy.is_shutdown():
            if self.joint_states and self.target_pose:
                ...
                #from stability analysis, the kp_gain for the jparse_singular direction and for position error must be equal
                stable_kp_gain_position = self.phi_gain_position #15.0; match position gain
                stable_kp_gain_angular = self.phi_gain_angular #15.0; match orientation gain
                stable_kd_gain_position = self.phi_gain_position/1.5 #7.0
                stable_kd_gain_angular = self.phi_gain_angular/2.5 #5.0
                ...
                if method == "JacobianPseudoInverse":
                    ...
                    if self.show_jparse_ellipses == True:
                        J_method, J_nullspace = self.jacobian_calculator.JParse(q=q, gamma=self.jparse_gamma, position_only=self.position_only, singular_direction_gain_position=stable_kp_gain_position, singular_direction_gain_angular=stable_kp_gain_angular,  jac_nullspace_bool=True, publish_jparse_ellipses=True, end_effector_pose=self.EndEffectorPose(q))
                        J_method_twist, J_nullspace_twist = self.jacobian_calculator.JParse(q=q, gamma=self.jparse_gamma, position_only=self.position_only, singular_direction_gain_position=stable_kd_gain_position, singular_direction_gain_angular=stable_kd_gain_angular,  jac_nullspace_bool=True, publish_jparse_ellipses=False, end_effector_pose=self.EndEffectorPose(q))
                    
                    else:
                        J_method, J_nullspace = self.jacobian_calculator.JParse(q=q, gamma=self.jparse_gamma, position_only=self.position_only, singular_direction_gain_position=stable_kp_gain_position, singular_direction_gain_angular=stable_kp_gain_angular,  jac_nullspace_bool=True)
                        J_method_twist, J_nullspace_twist = self.jacobian_calculator.JParse(q=q, gamma=self.jparse_gamma, position_only=self.position_only, singular_direction_gain_position=stable_kd_gain_position, singular_direction_gain_angular=stable_kd_gain_angular,  jac_nullspace_bool=True, publish_jparse_ellipses=False, end_effector_pose=self.EndEffectorPose(q))
                    
                    # J_method, J_nullspace = self.jacobian_calculator.JParse(q=q, gamma=0.1, position_only=self.position_only, singular_direction_gain=stable_kp_gain, jac_nullspace_bool=True) 
                    ...
                    J_method_twist = J_method
                elif method == "JacobianTranspose":
                    J_method, J_nullspace = self.jacobian_calculator.jacobian_transpose(q=q)
                    J_method_twist = J_method
                elif method == "JacobianNullspaceDecoupled":
                    J_method, J_nullspace = self.jacobian_calculator.jacobian_nullspace_decoupled(q=q, dq=dq, identity_weight_matrix=False)
                    J_method_twist = J_method
                #Find jacobian dot
                ...
                if self.is_sim == False:
                    ...
                    #simulation limits
                    error_norm_max = 0.05
                position_error, angular_error = self.pose_error(current_pose, target_pose, error_norm_max = error_norm_max)
                #log the pose error
                self.publish_pose_errors(position_error, angular_error)

                #get the mass matrix, coriolis, and gravity terms
                Mass = self.jacobian_calculator.inertia(q)
                Coriolis = self.jacobian_calculator.coriolis(q, dq)
                Gravity = self.jacobian_calculator.gravity(q)

                #Nullspace control objectives
                nominal_joint_motion = self.nominal_joint_pose_nullspace_panda(q, dq, Vmax = 0.5, Kp_value_set=5.0, Kd_value_set= 5.0) #VMax 50, Kp=5.0

                error_calc_time = rospy.Time.now() - t
                ...
                if self.position_only == True:
                    ...
                    # print("position error is:", position_error)
                    print("position error norm is:", np.linalg.norm(position_error))
                    end_effector_velocity = np.matrix(current_end_effector_velocity[:3]).T
                    kp_gain = stable_kp_gain_position
                    Kp = np.diag([kp_gain, kp_gain, kp_gain])  # Proportional gain matrix
                    kd_gain = 4.0
                    Kd = np.diag([kd_gain, kd_gain, kd_gain])  # Derivative gain matrix
                    task_vector = Kp @ position_error - Kd @ end_effector_velocity
                    # now calculate the joint velocities and joint torques
                    joint_velocities = J_method @ task_vector
                    # qddot = J_method @ task_vector - J_method @ J_dot[:3,:] @ dq_vect + J_nullspace @ nominal_joint_motion
                    qddot = J_method @ task_vector + J_nullspace @ nominal_joint_motion
                    joint_torque = Mass@qddot #+ Coriolis #+ Gravity
                else:
                    # Task position and orientation
                    kp_gain_position = stable_kp_gain_position
                    kp_gain_orientation = stable_kp_gain_angular
                    kd_gain_position = stable_kd_gain_position 
                    kd_gain_orientation = stable_kd_gain_angular

                    if method == "JacobianNullspaceDecoupled":
                        kp_gain_position = 40.0
                        kp_gain_orientation = 10.0
                        kd_gain_position = 10.0
                        kd_gain_orientation = 5.0
                    ...
                    task_pose = np.matrix(np.concatenate((task_position.T, task_orientation.T),axis=1)).T
                    #For velocities: twist of position and orientation
                    Kd_position = np.diag([kd_gain_position, kd_gain_position, kd_gain_position])  # Derivative gain matrix
                    Kd_orientation = np.diag([kd_gain_orientation, kd_gain_orientation, kd_gain_orientation])
                    task_position_vel = Kd_position @ np.matrix(current_end_effector_velocity[:3]).T
                    task_orientation_vel = Kd_orientation @ np.matrix(current_end_effector_velocity[3:]).T
                    task_twist = np.matrix(np.concatenate((task_position_vel.T, task_orientation_vel.T),axis=1)).T
                    # now add task pose and task twist
                    task_vector = task_pose - task_twist #this is Kp*(xd-x) - Kd*(dx)
                    task_vector_pose = task_pose
                    task_vector_twist = -task_twist
                    # Now calculate the joint velocities and joint torques
                    joint_velocities = J_method @ task_vector 
                    qddot = J_method @ task_vector_pose + J_method_twist @ task_vector_twist + J_nullspace @ nominal_joint_motion
                    joint_torque = Mass@qddot #+ Coriolis #+ Gravity
                    if q[3] > -0.1:
                        #if the robot is in a configuration where the middle joint is over-extended, add this term to bring it back
                        joint_torque[3] += -20.0 * (-0.1 - q[3]) - 0.5 * dq[3]
                        rospy.loginfo("WE ARE IN LOCKOUT Joint 3 torque: %f", joint_torque[3])
                ...
                # Create a dictionary for joint names and their corresponding torques and make the joint torques a list
                joint_torque_flattent_list = np.array(joint_torque).flatten().tolist()
                joint_torque_dict = dict(zip(self.joint_names, joint_torque_flattent_list))
                # Command the joint torques
                self.command_joint_torques(joint_torque_dict)
            self.rate.sleep()
            rospy.loginfo("Control loop running")

    def torque_home_robot(self):
        """
        This function commands the robot to the home position using joint torques.
        """
        # do the following in a loop for 5 seconds:
        t_start = rospy.Time.now()
        duration = 0.0
        while not rospy.is_shutdown() and duration < 5.0:
            ...
            if self.joint_states:
                ...
                for joint_name in self.joint_names:
                    ...
                    effort.append(self.joint_states.effort[idx])  
                self.current_positions = q
                # Home position for the Panda robot
                nominal_joint_motion = self.nominal_joint_pose_nullspace_panda(q, dq, Vmax = 50, homing=True)
                qddot = nominal_joint_motion
                Mass = self.jacobian_calculator.inertia(q)
                joint_torque = Mass@qddot #+ Coriolis #+ Gravity
                joint_torque_flattent_list = np.array(joint_torque).flatten().tolist()
                joint_torque_dict = dict(zip(self.joint_names, joint_torque_flattent_list))
    ...
    def command_joint_torques(self, joint_torque_dict):
        """
        This function iterates through the joint names and applies the corresponding torque to each joint.
        """
        # print("in command joint torques")
        try:
            for i, joint_name in enumerate(self.joint_names):
                # print("joint_name:", joint_name)
                # print("joint_torque_dict[joint_name]:", joint_torque_dict[joint_name])
                self.apply_torque(joint_name, joint_torque_dict[joint_name])
        except rospy.ServiceException as e:
            rospy.logerr(f"Failed to call /gazebo/apply_joint_effort: {e}")
